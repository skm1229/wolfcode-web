layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url: web.rootPath() + 'demoapp1/list',
        cellMinWidth: 95,
        page: true,
    ...
    table.on('toolbar(List-toolbar)', function (obj) {
        if (obj.event == 'add') {
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: web.rootPath() + 'demoapp1/add.html'
            });
        }
        ;
    });
        ...
        switch (obj.event) {
            case 'update':
                layer.open({
                    ...
                    fixed: false,
                    maxmin: true,
                    content: web.rootPath() + "demoapp1/" + data.id + ".html?_"
                });
                break;
            case 'delete':
                var index = layer.confirm('确定要删除?', {
                    ...
                    $.ajax({
                        url: web.rootPath() + "demoapp1/delete/" + data.id,
                        type: "delete",
